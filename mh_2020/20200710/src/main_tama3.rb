...
Window.loop do
  ...
  tamas.each do |tama|     ###■修正2-(3) 配列に入っている弾の１つ１つについて
    tama.move              ###■          弾を動かして
    tama.draw              ###■          弾を表示する
    if hit?(tama, bomb1)   ###■修正3-(1) 弾が爆弾１に命中したら
      bomb1.reset          ###■          爆弾１を消して
      tamas.delete(tama)   ###■          弾も消す
    end                    ###■
    if hit?(tama, bomb2)   ###■修正3-(2) 弾が爆弾２に命中したら
      bomb2.reset          ###■          爆弾２を消して
      tamas.delete(tama)   ###■          弾も消す
    end                    ###■
    if hit?(tama, bomb3)   ###■修正3-(3) 弾が爆弾３に命中したら
      bomb3.reset          ###■          爆弾３を消して
      tamas.delete(tama)   ###■          弾も消す
    end                    ###■
    if tama.centery < 0    ###■修正3-(4) 弾が画面の外まで行ったら
      tamas.delete(tama)   ###■          弾を消して
    end                    ###■
  end                      ###■
